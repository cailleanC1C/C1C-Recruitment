...
| **Welcome inactivity reminders** | `modules.onboarding.watcher_welcome` | Every 15 m | Scans welcome threads for incomplete onboarding, posts the 5h nudge and 24h warning, and auto-closes/renames threads at 36h to `Closed-W####-user-NONE` with a recruiter removal notice. | `c1c.onboarding.welcome_watcher` info/WARN lines for sends, rename/archive failures, and skipped targets. | `WELCOME_CHANNEL_ID`, FeatureToggles `welcome_dialog` and `recruitment_welcome`. |
