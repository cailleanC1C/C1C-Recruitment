...
- **Embeds & panels** — format the summary embed per [`docs/_meta/DocStyle.md`](../_meta/DocStyle.md), ping recruiter roles (e.g., `<@&RecruitmentCoordinator>`), and show inline status (waiting, saved, error) so staff can see what state the recruit is in.
...
2. Embed formatting follows the DocStyle rules: grouped sections, inline pairs (`**Power:** … • **Bracket:** …`), hide rules (`w_siege_detail` suppressed if Siege answer is "No"), and compact number formatting (K/M suffixes). See [`docs/_meta/DocStyle.md`](../_meta/DocStyle.md).
...
- **Watchers** — `modules/onboarding/watcher_welcome.py` binds Discord events, scheduling dedupe jobs for `WelcomeTickets`/`PromoTickets`, and notifies the runtime log channel described in [`docs/ops/Logging.md`](../ops/Logging.md).
- **Config & toggles** — [`docs/ops/Config.md`](../ops/Config.md) lists `WELCOME_TICKETS_TAB`, `PROMO_TICKETS_TAB`, `CLANLIST_TAB`, and feature toggles such as `WELCOME_ENABLED`, `ENABLE_WELCOME_HOOK`, and `welcome_dialog`. Welcome respects those toggles before wiring watchers at startup.
...
- **Summary embed:** Layout + hide rules follow [`docs/_meta/DocStyle.md`](../_meta/DocStyle.md). Number formatting shortens `w_power`, `w_hydra_clash`, `w_chimera_clash`, `w_cvc_points`. Inline pairs use the mid-dot (`•`) separator.
