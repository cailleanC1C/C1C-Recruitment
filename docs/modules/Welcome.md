...
- **Embeds & panels** — format the summary embed per [`docs/specs/Welcome_Summary_Spec.md`](../specs/Welcome_Summary_Spec.md), ping recruiter roles (e.g., `<@&RecruitmentCoordinator>`), and show inline status (waiting, saved, error) so staff can see what state the recruit is in.
...
- **Summary embed:** Layout + hide rules follow [`docs/specs/Welcome_Summary_Spec.md`](../specs/Welcome_Summary_Spec.md). Number formatting shortens `w_power`, `w_hydra_clash`, `w_chimera_clash`, `w_cvc_points`. Inline pairs use the mid-dot (`•`) separator.
