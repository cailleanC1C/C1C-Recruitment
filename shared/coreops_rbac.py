...
def get_admin_role_ids() -> Set[int]:
    ...
    ids: Set[int] = set()
    for tok in _parse_role_tokens(os.getenv("ADMIN_ROLE_IDS", "")):
        value = _safe_int(tok)
...
def is_staff_member(member: discord.abc.User | discord.Member) -> bool:
    ...
    if admin_role_ids.intersection(member_roles):
        return True
...
def is_admin_member(member: discord.abc.User | discord.Member) -> bool:
    ...
    if not admin_role_ids:
        return False
    return bool(admin_role_ids.intersection(_member_role_ids(member)))
